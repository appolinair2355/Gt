...
class SimpleRedirectionRestorer:
    ...
    def _get_user_phone(self, user_id, connections):
        ...
        try:
            # D'abord essayer depuis les sessions sauvegardées
            with open('user_data.json', 'r') as f:
                data = json.load(f)
            
            sessions = data.get('sessions', {})
            if str(user_id) in sessions:
                phone = sessions[str(user_id)].get('phone', '')
                if phone:
                    logger.info(f"Téléphone trouvé depuis les sessions sauvegardées: {phone}")
                    return phone.replace('+', '') if phone.startswith('+') else phone
            
            # Fallback vers les connexions classiques
    ...
    async def _create_telegram_client(self, user_id, phone_number):
        ...
        try:
            ...
            # D'abord chercher depuis les sessions sauvegardées
            saved_session = self._get_saved_session_file(user_id)
            session_files = []
            
            if saved_session:
                session_files.append(saved_session)
            
            # Ajouter les fichiers de session possibles
    ...
    def _get_saved_session_file(self, user_id):
        """Récupère le fichier de session sauvegardé"""
        try:
            with open('user_data.json', 'r') as f:
                data = json.load(f)
            ...
            sessions = data.get('sessions', {})
            if str(user_id) in sessions:
                session_file = sessions[str(user_id)].get('session_file', '')
                if session_file and os.path.exists(session_file):
                    logger.info(f"Session sauvegardée trouvée: {session_file}")
                    return session_file
            return None
        except:
            return None
